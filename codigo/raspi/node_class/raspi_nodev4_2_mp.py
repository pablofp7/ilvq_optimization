...
class RaspiNodev4_2:
    ...
    def __init__(self, id, dataset, modelo_proto, modelo_pred=None, share_protocol=None, recomendador=None, nodos=5, s=4, T=0.1, media_llegadas=0.1, puerto_base=10000, tam_colas = 1000):
        ...
        self.cola_protos = [deque(maxlen=tam_colas) for _ in range(self.nodos)]
        self.cola_index = 0
    ...
    def share(self):
        ...
        while True:
            ...
            if instruccion == "SHARE":
                ...
                print(f"El hilo emisor ha recibido: {instruccion}. Nodo: {self.id}.")
    ...
    def vaciar_cola_y_detener(self):
        # Vaciar la cola
        while not self.cola_signals.empty():
            try:
                self.cola_signals.get_nowait()  # Intenta obtener y descartar todos los elementos pendientes
            except queue.Empty:
                break  # Si la cola ya está vacía, rompe el bucle
    ...
    def recibir(self):
        ...
        timeout_s = 1
        server_socket.setsockopt(zmq.RCVTIMEO, int(timeout_s * 1000) )  # Establecer un tiempo de espera para el socket
