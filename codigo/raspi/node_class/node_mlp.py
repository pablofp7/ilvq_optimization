...
class MLPNodev1:
    ...
    def __init__(self, id, dataset, modelo, nodos=5, s=4, T=1.0, media_llegadas=0.1, puerto_base=10000):
        ...
        self.cola_params = self.manager.DequesProxy(num_deques = self.nodos, maxlen = 1)
       ...
       #Atributos para gestionar hilos/estadisticas
        ...
        self.proceso_receptor = multiprocessing.Process(target=self.recibir, args=(self.cola_params, self.nodos, self.puerto_base, self.id, self.s, self.T, self.fin_proceso)
        ...
        self.proceso_emisor = multiprocessing.Process(target=self.share, args=(self.id, self.puerto_base, self.vecinos, self.send_emisor, self.fin_proceso_emisor, 
                                                                                self.last_own_params, self.s, self.T, self.shared_times, self.tiempo_share, 
    ...
    def share(self, vecinos, send_emisor, fin_proceso_emisor, last_own_params, s, T, shared_times, tiempo_share, tiempo_no_share):
        """
        ...
        - last_own_params: Shared deque to store the last parameters of the model.
            ...
            while True:
                ...
                if fin_proceso_emisor.is_set():
                    ...
                    if random.random() < T:  # Probability check for sharing
                        ...
                        params_to_share = last_own_params.get(self.id, 0)  # Retrieve the last parameters
                        if params_to_share:
    ...
    def recibir(self, cola_model_params, nodos, puerto_base, id, s, T, fin_proceso):
        """
        Function for receiving model parameters asynchronously from neighbors.

        Args:
        - cola_model_params: A deque to store received model parameters.
        ...
        while True:
            try:
                ...
                cola_model_params.append(id_emisor, serialized_params)
