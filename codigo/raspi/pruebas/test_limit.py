...
kmeans_goal_sizes = [50, 75, 100, 150, 200, 250, 350, 500, 750, 1000]
...
for goal_size in kmeans_goal_sizes:
    df = read_dataset()
    df_list = [(fila[:-1], fila[-1]) for fila in df.values]

    modelo = MiILVQ(max_pset_size=goal_size, merge_mode="kmeans")
    matriz_conf = {"TP": 0, "TN": 0, "FP": 0, "FN": 0}
    train_time = 0
    prediction_time = 0
    train_operations = 0
    ...
    for i, (x, y) in enumerate(tqdm(df_list, desc=f"K-Means: Goal Size={goal_size}")):
        start_iteration_time = time.perf_counter_ns()
        x = {k: v for k, v in enumerate(x)}
    ...
    print(f"Finished: K-Means: Goal Size={goal_size}, Precision={precision}, Recall={recall}, F1={f1}, iteration time: {iteration_time / 1e9}")
    print(f"Matriz de confusion: {matriz_conf}")
